...
def _dynamic_step(base: dict, index: int, steps: int) -> dict:
    ...
    data["Engine_RPM"] *= 1.0 + 0.012 * math.sin(
        phase * 3.0
    )

    data["Fuel_Flow"] *= 1.0 + 0.018 * math.sin(
        phase * 2.0 + 0.4
    )
    ...
    for key, offset in [
        ...
        data[key] *= 1.0 + 0.006 * math.sin(
            phase * 2.5 + offset
        )

    data["Oil_Temp"] *= 1.0 + 0.004 * math.sin(
        phase
    )

    data["MAP_Injector"] *= 1.0 + 0.008 * math.sin(
        phase * 1.5
    )
...
def run_replay(
        ...
        timeline.append(
            {
                ...
                "health_state": analysis[
                    "health_state"
                ],
                "health_index": analysis[
                    "health_index"
                ],
                "anomaly_score": analysis[
                    "anomaly_score"
                ],
                "residual_rms": round(
                    float(
    ...
    return {
        ...
        "summary": {
            ...
            "comparison_note":
                "The intelligent warning fuses model/twin "
                "evidence; the reference alarm is a 3-sigma "
                "single-parameter Digital-Twin baseline. "
                "Neither is a certified engine limit.",

            "final_health_index":
                timeline[-1]["health_index"],

            "final_health_state":
                timeline[-1]["health_state"],
            ...
            "initial_rul_hours":
                timeline[0]["rul_hours"],

            "final_rul_hours":
                timeline[-1]["rul_hours"],
            ...
            "rul_note":
                "RUL preferentially follows the observed "
                "replay health trajectory when a finite "
                "trend estimate is available; otherwise it "
                "falls back to the feature-based methodology "
                "model. This is not validated run-to-failure "
                "RUL for a deployed MALE-UAV engine.",
        },
    }