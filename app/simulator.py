...
def mission_adjust(
    ...
    data = copy.deepcopy(telemetry)
    altitude_factor = max(0.0, float(altitude_ft)) / 10000
    hot_factor = max(0.0, float(ambient_c) - 25) / 25
    endurance_factor = max(0.0, float(duration_h) - 4) / 8

    # Demonstrator response functions only. Replace with validated aero-piston
    # engine maps/thermodynamic equations when target-engine data are available.
    ...
    if "CHT" in data:
        data["CHT"] *= 1 + 0.02 * altitude_factor + 0.04 * hot_factor + 0.02 * endurance_factor
    if "Alternator_Temp" in data:
        data["Alternator_Temp"] *= 1 + 0.015 * hot_factor + 0.01 * endurance_factor
