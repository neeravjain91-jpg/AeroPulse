...
class AeroTwinAI:
    ...
    def analyze(self, telemetry: dict, context: dict | None = None):
        ...
        health_index_value = max(0.0, min(100.0, 100.0 - HEALTH_ORDER.get(prediction, 1) * 18.0 - min(float(twin["residual_rms"]), 12.0) * 4.0 - max(0.0, 100.0 - sensor_health["overall_trust_score"]) * 0.10))
        if health_index_value >= 85: fused_state = "Normal"
        elif health_index_value >= 65: fused_state = "Watch"
        elif health_index_value >= 40: fused_state = "Warning"
        ...
        return {
            ...
            "health_index": round(health_index_value, 1), "anomaly_score": round(anomaly_score, 4),
